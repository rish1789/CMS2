...
/** 022: combines 020's staff-token fixture and 021's Patient Account fixture with a Queue-mode Session helper. */
@SpringBootTest
...
public abstract class AbstractQueueBookingIntegrationTest {
    ...
    @Autowired
    protected StaffQueueBookingService staffQueueBookingService;

    @Autowired
    protected PatientQueueBookingService patientQueueBookingService;
    ...
    /** An active Queue/Token Session (every day) for a doctor staffed at the given clinic. */
    ...
    /** A Fixed-Time Session, for asserting queue booking rejects non-Queue sessions. */
    protected Session saveFixedTimeSession(Clinic clinic, DoctorProfile doctor) {
        Schedule schedule = scheduleRepository.save(new Schedule(
