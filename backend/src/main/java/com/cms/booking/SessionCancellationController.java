...
/** 029: Operations-or-ClinicAdmin only (never the Doctor) - the standard write-action gate, not 026/027's view-only unrestricted one. */
@RestController
public class SessionCancellationController {
    ...
    public SessionCancellationController(
            SessionRepository sessionRepository,
            RoleAssignmentRepository roleAssignmentRepository,
            SessionCancellationService sessionCancellationService) {
        this.sessionRepository = sessionRepository;
        this.roleAssignmentRepository = roleAssignmentRepository;
        this.sessionCancellationService = sessionCancellationService;
    }

    @PostMapping("/api/v1/clinics/{clinicId}/sessions/{sessionId}/cancel")
    public SessionCancellationResponse cancel(
        ...
        int cancelledCount = sessionCancellationService.cancelSession(session);
        return new SessionCancellationResponse(sessionId, cancelledCount);
    }

    /** Mirrors 016/020/025/028's identical Operations-or-ClinicAdmin write-action gate. */
